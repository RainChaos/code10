...
public interface LeaveMapper {
    ...
    //
    @Select("select * from [leave] lea left join teacher tea on tea.te_no = lea.teNo where tea.sc_code=#{scCode} and lea.stuNo=#{stuNo} and " +
            "((lea.startDate >=#{startDate} and lea.startDate<= #{endDate}) " +
            "or (lea.endDate <=#{startDate} and lea.endDate>= #{endDate}))")
    Leave findOneByStuNoAndTime(@Param("scCode") String scCode,@Param("stuNo") String stuNo, @Param("startDate") Date startDate, @Param("endDate") Date endDate);
